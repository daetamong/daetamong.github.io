...
$SIM(x, y) = \frac{sum _{i=1}^n (x_{i} - \bar x)(y_{i} - \bar y)}{\sqrt{sum _{i=1}^n (x_{i} - \bar x)^2 sum_{i=1}^n (y_{i} - \bar y)^2}}$
...
$Cos(\Theta) = \frac{A \cdot B}{\Vert A \Vert \Vert B \Vert}$ = $\frac{sum _{i=1}^n A_{i} \times B_{i}}{{\sqrt{sum _{i=1}^n (A_{i}^2)}} \times \sqrt{sum _{i=1}^n (B_{i}^2)}}$
