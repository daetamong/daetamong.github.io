...
아까 우리는 각 메소드마다 어떤 텍스트가 출력이 되는지 확인을 했었다. 위의 설명대로라면 person.age를 출력했을 때 나이만 출력이 되어야겠지만

위의 결과를 보면 set_age() 메소드가 실행되고 그 다음 get_age() 메소드가 실행되는 것과 같은 것을 알 수 있다.

다시 말해서 person.age를 실행했다는 것은 곧, set_age()가 실행됐다는 것과 같다.

하지만 바로 위에서 만든 클래스에서는 set_age가 아니라 age라는 메소드 위에 @age.setter라는 데코레이터를 붙여줬다.

이는 곧 @age.setter가 set_age() 메소드와 같은 Setter 메소드를 만들어 준다고 보면 되곘다.



### 정리를 하자면..?
> 1. **"{클래스 인스턴스}.{메소드 이름}"**을 실행했을 때 @property가 붙은 메소드가 실행된다.
> 2. **"{클래스 인스턴스}.{메소드 이름} = 변경하고자 하는 값"**을 실행했을 때, @{메소드 이름}.setter가 붙은 메소드가 실행된다
